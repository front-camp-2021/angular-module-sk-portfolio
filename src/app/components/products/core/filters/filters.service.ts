...
export class FiltersService {
  ...
  removeActiveCategoryField(value: string) {
    return (this.activeCategoryFields = this.activeCategoryFields.filter(
      (activeCategoryField) => activeCategoryField !== value
    ));
  }
  ...
  removeActiveBrandField(value: string) {
    return (this.activeBrandFields = this.activeBrandFields.filter(
      (activeBrandField) => activeBrandField !== value
    ));
  }
  ...
  setFieldsState(newField: Field) {
    const fieldsStateValid = this.fieldsState.some(
      (field: Field) => field.value === newField.value
    );
