...
export class CardlistContainerComponent implements OnInit {
  ...
  ngDoCheck(): void {
    ...
    this.activeCategoriesFilters =
      this.filtersService.getActiveCategoryFilters();
    this.slidersValues = this.filtersService.getSlidersValue();

    if (
      this.activeBrandFilters.length !== 0 ||
      this.activeCategoriesFilters.length !== 0 ||
      ...
      this.cardlist = this.productStorageService.setProducts(
        this.searchValue,
    ...
    this.cardlistFromRange = this.productStorageService.getProductsFromRange(
      this.cardlist,
      this.slidersValues
    );
